...
运行:
  .venv/bin/python implementation/tests/mobile/test_voice_e2e.py
"""
...
async def main():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("http://localhost:3001/api/stt", data=data) as resp:
                result = await resp.json()
                if result.get("text"):
                    print(f"  [✓] 风铃 STT: \"{result.get('text')}\"")
                else:
                    print(f"  [✗] 风铃 STT: {result}")
    except Exception as e:
        print(f"  [✗] 风铃 STT: {e}")
