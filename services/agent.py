...
class LocalAgent:
    ...
    def __init__(
        ...
        self._workspace = Path(workspace_dir or os.environ.get("WORKSPACE_DIR", "persona"))
        if not self._workspace.is_absolute():
            self._workspace = Path(os.environ.get("DEVKIT_DIR", str(Path(__file__).parent.parent))) / self._workspace
